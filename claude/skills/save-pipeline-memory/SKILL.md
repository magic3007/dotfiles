...
- 加上 `--help` / `--dry-run` 支持，让使用者能快速理解用法
- 确保另一人拿到脚本 + 数据就能复现，无需从 memory 里复制粘贴

**不要在 memory 里记录手拼的裸命令**（如 `tool submit --flag1 --flag2 ...`）。如果你写出的是裸命令而非脚本调用，说明第 2 步没完成。

**什么情况下可以不写脚本？** 仅当步骤是：
- 纯手动的一次性交互操作（如网页点击、手动审批）
- 浏览/查看类操作（如 `cat`、`ls`、`head` 查看结果）

即使如此，也要优先考虑是否可以用脚本 + API 替代手动操作。
...
- **脚本化强制（不可妥协）**：Pipeline 节每一步是脚本调用，不允许裸命令。这是硬约束，不是建议。
- **可独立运行**：脚本不依赖 session 上下文、不依赖 memory 里的复制粘贴，拿到就能跑。
- **环境无关**：不假定特定工具名（如 mmctl/vela），用通用描述或从配置读取。

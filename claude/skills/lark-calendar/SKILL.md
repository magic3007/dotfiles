...
> Tip: read this skill's own files (e.g. `references/...`) with `lark-cli skills read lark-calendar <relative-path>` to keep them in sync with this CLI version. A reference to another skill (`../lark-foo/...`) uses the same command with the leading `../` removed: `lark-cli skills read lark-foo/...`.
